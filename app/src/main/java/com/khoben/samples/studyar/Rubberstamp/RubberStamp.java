...
package com.khoben.samples.studyar.Rubberstamp;
...
public class RubberStamp {
    ...
    private void addTextToBitmap(@NonNull RubberStampConfig config,
        ...
        for (String str : config.getRubberStampString().keySet()) {
            ...
            float rubberStampMeasuredWidth = paint.measureText(str);
            int rubberStampHeight = bounds.height();

            int positionX = config.getPositionX();
            int positionY = config.getPositionY() + startH;

            startH += config.getTextSize();
            ...
            if (config.getRubberStampPosition() != TILE) {
                ...
                canvas.drawText(str, positionX, positionY, paint);
            } else {
                ...
                textCanvas.drawText(str, 0, rubberStampHeight, paint);
